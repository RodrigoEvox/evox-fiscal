...
import {
  ...
  InsertSetores,
  usuarioSetores,
  InsertUsuarioSetores,
  tarefas,
  InsertTarefas,
  tarefaComentarios,
  InsertTarefaComentarios,
  arquivos,
  InsertArquivos,
  auditLog,
  InsertAuditLogs,
  ...
  InsertSetorConfigs,
  modelosParceria,
  InsertModelosParceria,
  comissoesServico,
  InsertComissoesServico,
  parceiroServicos,
  InsertParceiroServicos,
  slaConfiguracoes,
  InsertSlaConfiguracoes,
  servicoEtapas,
  InsertServicoEtapas,
  clienteServicos,
  InsertClienteServicos,
  executivosComerciais,
  InsertExecutivosComerciais,
  rateioComissao,
  InsertRateioComissoes,
  aprovacaoComissao,
  userHistory,
  InsertUserHistories,
  chatMessages,
  ...
  InsertChatChannels,
  chatNotifications,
  InsertChatNotifications,
  chatReactions,
  InsertChatReactions,
  chatTypingIndicators,
  InsertChatTypingIndicators,
  colaboradores,
  InsertColaboradores,
  ferias,
  InsertFeriases,
  solicitacoesFolga,
  InsertSolicitacoesFolga,
  tarefasSetor,
  InsertTarefasSetor,
  acoesBeneficios,
  InsertAcoesBeneficios,
  atestadosLicencas,
  InsertAtestadosLicencas,
  planosCarreira,
  InsertPlanosCarreira,
  historicoStatusColaborador,
  InsertHistoricoStatusColaboradores,
  onboardingTemplates,
  InsertOnboardingTemplates,
  onboardingEtapasTemplate,
  InsertOnboardingEtapasTemplate,
  onboardingColaborador,
  InsertOnboardingColaboradores,
  onboardingEtapas,
  InsertOnboardingEtapas,
  pesquisasClima,
  InsertPesquisasClima,
  perguntasClima,
  InsertPerguntasClima,
  respostasClima,
  InsertRespostasClima,
  bancoHoras,
  InsertBancoHoras,
  userPresence,
  InsertUserPresences,
  valeTransporte,
  InsertValeTransportes,
  academiaBeneficio,
  InsertAcademiaBeneficio,
  comissaoRh,
  InsertComissoesRh,
  dayOff,
  InsertDaysOff,
  ...
  InsertDoacoesSangue,
  reajustesSalariais,
  InsertReajustesSalariais,
  apontamentosFolha,
  InsertApontamentosFolha,
  niveisCargo,
  InsertNiveisCargo,
  beneficiosCustom,
  InsertBeneficiosCustom,
  programasCarreira,
  InsertProgramasCarreira,
  rescisoes,
  InsertRescisoes,
  equipamentosColaborador,
  InsertEquipamentosColaborador,
  senhasAutorizacoes,
  InsertSenhasAutorizacoes,
  emailsCorporativos,
  InsertEmailsCorporativos,
  senhaHistorico,
  InsertSenhasHistorico,
...
export async function upsertUser(user: InsertUsers): Promise<void> {
  if (!user.openId) throw new Error("User openId is required for upsert");
  const db = await getDb();
  if (!db) return;

  const values: InsertUsers = { openId: user.openId };
...
export async function createUser(data: InsertUsers): Promise<number> {
  const db = await getDb();
  if (!db) return 0;
...
export async function updateUserRole(
  ...
  await db
    .update(users)
    .set({ role: role as any, nivelAcesso: nivelAcesso as any })
    .where(eq(users.id, id));
}

export async function updateUser(id: number, data: Partial<InsertUsers>) {
  const db = await getDb();
  if (!db) return;
...
export async function createSetor(data: InsertSetores) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateSetor(id: number, data: Partial<InsertSetores>) {
  const db = await getDb();
  if (!db) return;
...
export async function listUsuarioSetores(userId?: number) {
  ...
  if (userId) {
    return db
      .select()
      .from(usuarioSetores)
      .where(eq(usuarioSetores.userId, userId));
  }
  return db.select().from(usuarioSetores);
}

export async function addUsuarioSetor(data: InsertUsuarioSetores) {
  const db = await getDb();
  if (!db) return null;
...
export async function removeUsuarioSetor(userId: number, setorId: number) {
  ...
  if (!db) return;
  await db
    .delete(usuarioSetores)
    .where(
      and(
        eq(usuarioSetores.userId, userId),
        eq(usuarioSetores.setorId, setorId)
      )
    );
}
...
export async function createParceiro(data: InsertParceiros) {
  const db = await getDb();
  if (!db) return null;
...
export async function createCliente(data: InsertClientes) {
  const db = await getDb();
  if (!db) return null;
  ...
  await db
    .update(clientes)
    .set({ codigo } as any)
    .where(eq(clientes.id, insertId));
  return { id: insertId, codigo };
}

export async function updateCliente(id: number, data: Partial<InsertClientes>) {
  const db = await getDb();
  if (!db) return;
...
export async function createTese(data: InsertTeses) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateTese(id: number, data: Partial<InsertTeses>) {
  const db = await getDb();
  if (!db) return;
...
export async function createFilaItem(data: InsertFilaApuracoes) {
  const db = await getDb();
  if (!db) return null;
  const maxOrder = await db
    .select({ max: sql<number>`COALESCE(MAX(ordem), 0)` })
    .from(filaApuracao)
    .where(eq(filaApuracao.status, data.status || "a_fazer"));
  const nextOrder = (maxOrder[0]?.max || 0) + 1;
  const result = await db
    .insert(filaApuracao)
    .values({ ...data, ordem: nextOrder });
  return result[0].insertId;
}

export async function updateFilaItem(
  id: number,
  data: Partial<InsertFilaApuracoes>
) {
  const db = await getDb();
  if (!db) return;
...
export async function getRelatoriosByClienteId(clienteId: number) {
  ...
  return db
    .select()
    .from(relatorios)
    .where(eq(relatorios.clienteId, clienteId))
    .orderBy(desc(relatorios.createdAt));
}

export async function createRelatorio(data: InsertRelatorios) {
  const db = await getDb();
  if (!db) return null;
...
export async function createNotificacao(data: InsertNotificacoes) {
  const db = await getDb();
  if (!db) return null;
  try {
    ...
    if (
      err?.cause?.code === "WARN_DATA_TRUNCATED" ||
...
export async function listTarefas(filters?: {
  setorId?: number;
  ...
  if (filters?.clienteId)
    conditions.push(eq(tarefas.clienteId, filters.clienteId));
  ...
  if (conditions.length > 0) {
    return db
      ...
      .orderBy(asc(tarefas.ordem), desc(tarefas.createdAt));
  }
  return db
    .select()
    .from(tarefas)
...
export async function getSubtarefas(tarefaPaiId: number) {
  ...
  return db
    .select()
    .from(tarefas)
    .where(eq(tarefas.tarefaPaiId, tarefaPaiId))
    .orderBy(asc(tarefas.ordem));
}

export async function createTarefa(data: InsertTarefas) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateTarefa(id: number, data: Partial<InsertTarefas>) {
  const db = await getDb();
  if (!db) return;
...
export async function createComentario(data: InsertTarefaComentarios) {
  const db = await getDb();
  if (!db) return null;
...
export async function listArquivos(entidadeTipo?: string, entidadeId?: number) {
  ...
  const conditions = [];
  if (entidadeTipo)
    conditions.push(eq(arquivos.entidadeTipo, entidadeTipo as any));
...
export async function createArquivo(data: InsertArquivos) {
  const db = await getDb();
  if (!db) return null;
...
export async function createAuditEntry(data: InsertAuditLogs) {
  const db = await getDb();
  if (!db) return null;
...
export async function createApiKey(data: InsertApiKeys) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateApiKey(id: number, data: Partial<InsertApiKeys>) {
  const db = await getDb();
  if (!db) return;
...
export async function listServicos(setorId?: number) {
  ...
  if (setorId) {
    return db
      .select()
      .from(servicos)
      .where(eq(servicos.setorId, setorId))
      .orderBy(asc(servicos.nome));
  }
  return db.select().from(servicos).orderBy(asc(servicos.nome));
}

export async function createServico(data: InsertServicos) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateServico(id: number, data: Partial<InsertServicos>) {
  const db = await getDb();
  if (!db) return;
...
export async function getSetorConfigBySetorId(setorId: number) {
  ...
  const result = await db
    .select()
    .from(setorConfig)
    .where(eq(setorConfig.setorId, setorId))
    .limit(1);
  return result.length > 0 ? result[0] : null;
}
...
export async function getSetorConfigBySigla(sigla: string) {
  ...
  const result = await db
    .select()
    .from(setorConfig)
    .where(eq(setorConfig.sigla, sigla))
    .limit(1);
  return result.length > 0 ? result[0] : null;
}

export async function upsertSetorConfig(data: InsertSetorConfigs) {
  const db = await getDb();
  if (!db) return null;
  // Check if exists
  ...
  if (existing) {
    await db
      .update(setorConfig)
      .set(data)
      .where(eq(setorConfig.id, existing.id));
    return existing.id;
  }
...
export async function buscaGlobal(termo: string) {
  ...
  const parceirosResult = await db
    .select({
      id: parceiros.id,
  ...
  const tesesResult = await db
    .select({
      id: teses.id,
      nome: teses.nome,
      tributoEnvolvido: teses.tributoEnvolvido,
    })
    .from(teses)
  ...
  const usuariosResult = await db
    ...
    .from(users)
    .where(
      or(sql`${users.name} LIKE ${like}`, sql`${users.email} LIKE ${like}`)
    )
    .limit(10);

  const livrosResult = await db
    .select({
      id: bibLivros.id,
      titulo: bibLivros.titulo,
      autores: bibLivros.autores,
      isbn: bibLivros.isbn,
...
export async function seedSetoresEvox() {
  ...
  const setoresData = [
    {
      nome: "SPC – Suporte Comercial",
      descricao: "Suporte comercial e gestão de parcerias",
      cor: "#F59E0B",
      icone: "Handshake",
      sigla: "SPC",
      submenus: [
        {
          key: "nova-tarefa",
          label: "Nova Tarefa",
          rota: "/setor/spc/nova-tarefa",
        },
        {
          key: "parcerias",
          label: "Gestão de Parcerias",
          rota: "/setor/spc/parcerias",
        },
      ],
    },
    {
      nome: "RCT – Crédito",
      descricao: "Recuperação de créditos tributários",
      cor: "#10B981",
      ...
      submenus: [
        {
          key: "nova-tarefa",
          label: "Nova Tarefa",
          rota: "/setor/rct/nova-tarefa",
        },
        { key: "fila", label: "Fila de Apuração RCT", rota: "/setor/rct/fila" },
        { key: "teses", label: "Teses Tributárias", rota: "/setor/rct/teses" },
        {
          key: "analitica",
          label: "Visão Analítica RCT",
          rota: "/setor/rct/analitica",
        },
      ],
    },
    {
      nome: "DPT – Transação",
      descricao: "Departamento de transação tributária",
      cor: "#8B5CF6",
      ...
      submenus: [
        {
          key: "nova-tarefa",
          label: "Nova Tarefa",
          rota: "/setor/dpt/nova-tarefa",
        },
        { key: "fila", label: "Fila de Apuração DPT", rota: "/setor/dpt/fila" },
        {
          key: "analitica",
          label: "Visão Analítica DPT",
          rota: "/setor/dpt/analitica",
        },
      ],
    },
    {
      ...
      submenus: [
        {
          key: "nova-tarefa",
          label: "Nova Tarefa",
          rota: "/setor/jur/nova-tarefa",
        },
        { key: "fila", label: "Fila de Execução JUR", rota: "/setor/jur/fila" },
      ],
    },
    {
      nome: "RT – Reforma Tributária",
      descricao: "Reforma tributária e consultoria",
      cor: "#06B6D4",
      icone: "Landmark",
      sigla: "RT",
      submenus: [
        {
          key: "simulador",
          label: "Simulador de Impactos",
          rota: "/setor/rt/simulador",
        },
        {
          key: "consultoria",
          label: "Consultoria",
          rota: "/setor/rt/consultoria",
        },
      ],
    },
    {
      ...
      submenus: [
        {
          key: "dashboard-contratos",
          label: "Dashboard",
          rota: "/contratos/dashboard",
          grupo: "Gestão",
        },
        {
          key: "novo-contrato",
        ...
        },
        {
          key: "fila-elaboracao",
          label: "Elaboração",
          rota: "/contratos/fila/elaboracao",
          grupo: "Filas",
        },
        {
          key: "fila-revisao",
          label: "Revisão",
          rota: "/contratos/fila/revisao",
          grupo: "Filas",
        },
        {
          key: "fila-assinatura",
          label: "Assinatura",
          rota: "/contratos/fila/assinatura",
          grupo: "Filas",
        },
        {
          key: "fila-vigencia",
          label: "Vigência",
          rota: "/contratos/fila/vigencia",
          grupo: "Filas",
        },
        {
          key: "fila-renovacao",
          label: "Renovação",
          rota: "/contratos/fila/renovacao",
          grupo: "Filas",
        },
        {
          key: "fila-encerrado",
          label: "Encerrados",
          rota: "/contratos/fila/encerrado",
          grupo: "Filas",
        },
      ],
    },
      ...
      submenus: [
        {
          key: "nova-tarefa",
          label: "Nova Tarefa",
          rota: "/setor/fin/nova-tarefa",
        },
        {
          key: "contas-pagar",
          label: "Contas a Pagar",
          rota: "/setor/fin/contas-pagar",
        },
        {
          key: "contas-receber",
          label: "Contas a Receber",
          rota: "/setor/fin/contas-receber",
        },
        {
          key: "contas-bancarias",
          label: "Contas Bancárias",
          rota: "/setor/fin/contas-bancarias",
        },
      ],
    },
    {
      ...
      submenus: [
        {
          key: "nova-tarefa",
          label: "Nova Tarefa",
          rota: "/setor/mkt/nova-tarefa",
        },
        {
          key: "redes-sociais",
          label: "Redes Sociais",
          rota: "/setor/mkt/redes-sociais",
        },
      ...
      submenus: [
        {
          key: "nova-tarefa",
          label: "Nova Tarefa",
          rota: "/setor/rh/nova-tarefa",
        },
        {
          key: "colaboradores",
          label: "Colaboradores",
          rota: "/setor/rh/colaboradores",
        },
        { key: "ferias", label: "Férias", rota: "/setor/rh/ferias" },
        {
          key: "vale-transporte",
          label: "Vale Transporte",
          rota: "/setor/rh/vale-transporte",
        },
        { key: "academia", label: "Academia", rota: "/setor/rh/academia" },
        {
          key: "comissao-rh",
          label: "Comissão RH",
          rota: "/setor/rh/comissao-rh",
        },
        { key: "day-off", label: "Day Off", rota: "/setor/rh/day-off" },
        {
          key: "doacao-sangue",
          label: "Doação de Sangue",
          rota: "/setor/rh/doacao-sangue",
        },
        {
          key: "reajustes",
          label: "Reajustes Salariais",
          rota: "/setor/rh/reajustes",
        },
        {
          key: "apontamentos-folha",
          label: "Apontamentos Folha",
          rota: "/setor/rh/apontamentos-folha",
        },
        {
          key: "niveis-cargo",
          label: "Níveis de Cargo",
          rota: "/setor/rh/niveis-cargo",
        },
      ],
    },
  ];
  ...
  for (const s of setoresData) {
    ...
    const existing = await db
      .select()
      .from(setores)
      .where(sql`${setores.nome} LIKE ${`${sigla}%`}`)
      .limit(1);
    let setorId: number;
    ...
    if (existingConfig.length > 0) {
      await db
        ...
        .set({
          sigla,
          submenus: submenus as any,
          workflowStatuses: defaultWorkflow,
        })
        .where(eq(setorConfig.id, existingConfig[0].id));
    } else {
      await db
        .insert(setorConfig)
        .values({
...
export async function listModelosParceria() {
  ...
  return db
    .select()
    .from(modelosParceria)
    .where(eq(modelosParceria.ativo, true))
    .orderBy(asc(modelosParceria.ordem));
}

export async function createModeloParceria(data: InsertModelosParceria) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateModeloParceria(
  id: number,
  data: Partial<InsertModelosParceria>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listComissoesByServico(servicoId: number) {
  ...
  return db
    .select()
    .from(comissoesServico)
    .where(eq(comissoesServico.servicoId, servicoId));
}
...
export async function listComissoesByModelo(modeloParceriaId: number) {
  ...
  return db
    .select()
    .from(comissoesServico)
    .where(eq(comissoesServico.modeloParceriaId, modeloParceriaId));
}

export async function upsertComissaoServico(
  servicoId: number,
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(comissoesServico.servicoId, servicoId),
        eq(comissoesServico.modeloParceriaId, modeloParceriaId)
      )
    )
    .limit(1);
  if (existing.length > 0) {
    await db
      .update(comissoesServico)
      .set({ percentualComissao })
      .where(eq(comissoesServico.id, existing[0].id));
  } else {
    await db
      .insert(comissoesServico)
      .values({ servicoId, modeloParceriaId, percentualComissao });
  }
}
...
export async function listParceiroServicos(parceiroId: number) {
  ...
  return db
    .select()
    .from(parceiroServicos)
    .where(eq(parceiroServicos.parceiroId, parceiroId));
}

export async function addParceiroServico(data: InsertParceiroServicos) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateParceiroServico(
  id: number,
  data: Partial<InsertParceiroServicos>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(parceiroServicos)
    .set(data)
    .where(eq(parceiroServicos.id, id));
}
...
export async function listSlaConfiguracoes(setorId?: number) {
  ...
  if (setorId) {
    return db
      .select()
      .from(slaConfiguracoes)
      .where(eq(slaConfiguracoes.setorId, setorId))
      .orderBy(asc(slaConfiguracoes.nome));
  }
  return db.select().from(slaConfiguracoes).orderBy(asc(slaConfiguracoes.nome));
}

export async function createSlaConfiguracao(data: InsertSlaConfiguracoes) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateSlaConfiguracao(
  id: number,
  data: Partial<InsertSlaConfiguracoes>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(slaConfiguracoes)
    .set(data)
    .where(eq(slaConfiguracoes.id, id));
}
...
export async function listServicoEtapas(servicoId: number) {
  ...
  return db
    .select()
    .from(servicoEtapas)
    .where(eq(servicoEtapas.servicoId, servicoId))
    .orderBy(asc(servicoEtapas.ordem));
}

export async function createServicoEtapa(data: InsertServicoEtapas) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateServicoEtapa(
  id: number,
  data: Partial<InsertServicoEtapas>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listClienteServicos(clienteId: number) {
  ...
  return db
    .select()
    .from(clienteServicos)
    .where(eq(clienteServicos.clienteId, clienteId));
}

export async function addClienteServico(data: InsertClienteServicos) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateClienteServico(
  id: number,
  data: Partial<InsertClienteServicos>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listSubparceiros(parceiroPaiId: number) {
  ...
  return db
    .select()
    .from(parceiros)
    .where(eq(parceiros.parceiroPaiId, parceiroPaiId))
...
export async function syncParceiroServicos(
  parceiroId: number,
  servicoIds: number[]
) {
  const db = await getDb();
...
export async function listExecutivos() {
  ...
  return db
    .select()
    .from(executivosComerciais)
    .orderBy(asc(executivosComerciais.nome));
}
...
export async function getExecutivoById(id: number) {
  ...
  const result = await db
    .select()
    .from(executivosComerciais)
    .where(eq(executivosComerciais.id, id))
    .limit(1);
  return result.length > 0 ? result[0] : null;
}

export async function createExecutivo(data: InsertExecutivosComerciais) {
  const db = await getDb();
  if (!db) return null;
  const result = await db
    .insert(executivosComerciais)
    .values(data)
    .$returningId();
  return result[0]?.id;
}

export async function updateExecutivo(
  id: number,
  data: Partial<InsertExecutivosComerciais>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(executivosComerciais)
    .set(data)
    .where(eq(executivosComerciais.id, id));
}
...
export async function listRateioByParceiro(parceiroId: number) {
  ...
  return db
    .select()
    .from(rateioComissao)
    .where(eq(rateioComissao.parceiroId, parceiroId));
}

export async function upsertRateio(data: InsertRateioComissoes) {
  const db = await getDb();
  if (!db) return null;
  // Check if exists
  const existing = await db
  ...
  if (existing.length > 0) {
    await db
      .update(rateioComissao)
      .set(data)
      .where(eq(rateioComissao.id, existing[0].id));
...
export async function createAprovacaoComissao(
  data: typeof aprovacaoComissao.$inferInsert
) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateAprovacaoStatus(
  id: number,
...
export async function createUserHistoryEntry(data: InsertUserHistories) {
  const db = await getDb();
  if (!db) return null;
...
export async function getChatChannel(id: number) {
  ...
  const rows = await db
    .select()
    .from(chatChannels)
    .where(sql`${chatChannels.id} = ${id}`);
  return rows[0] || null;
}

export async function createChatChannel(data: InsertChatChannels) {
  const db = await getDb();
  if (!db) return null;
...
export async function createChatMessage(data: InsertChatMessages) {
  const db = await getDb();
  if (!db) return null;
...
export async function listChatMessages(
  ...
  if (beforeId) {
    return db
      ...
      .from(chatMessages)
      .where(
        sql`${chatMessages.channelId} = ${channelId} AND ${chatMessages.id} < ${beforeId}`
      )
...
export async function clearChatChannel(
  channelId: number,
  deletedByUserId: number
) {
  const db = await getDb();
...
export async function updateChatChannel(
  ...
  if (data.descricao !== undefined)
    sets.push(`descricao = '${(data.descricao || "").replace(/'/g, "''")}'`);
  if (sets.length > 0) {
    await db.execute(
      sql.raw(
        `UPDATE chat_channels SET ${sets.join(", ")} WHERE id = ${channelId}`
      )
    );
  }
}
...
export async function searchChatMessages(query: string, channelId?: number) {
  ...
  if (channelId) {
    return db
      ...
      .from(chatMessages)
      .where(
        sql`${chatMessages.content} LIKE ${like} AND ${chatMessages.channelId} = ${channelId} AND ${chatMessages.deletedAt} IS NULL`
      )
      .orderBy(desc(chatMessages.id))
      .limit(50);
  }
  return db
    ...
    .from(chatMessages)
    .where(
      sql`${chatMessages.content} LIKE ${like} AND ${chatMessages.deletedAt} IS NULL`
    )
    .orderBy(desc(chatMessages.id))
    .limit(50);
}
...
export async function addReaction(data: InsertChatReactions) {
  const db = await getDb();
  if (!db) return null;
  ...
  const existing = await db
    ...
    .from(chatReactions)
    .where(
      sql`${chatReactions.messageId} = ${data.messageId} AND ${chatReactions.userId} = ${data.userId} AND ${chatReactions.emoji} = ${data.emoji}`
    );
...
export async function getReactionsForMessages(messageIds: number[]) {
  ...
  const result = await db.execute(
    sql.raw(
      `SELECT * FROM chat_reactions WHERE messageId IN (${idList}) ORDER BY createdAt ASC`
    )
  );
...
export async function getPinnedMessages(channelId: number) {
  ...
  return db
    ...
    .from(chatMessages)
    .where(
      sql`${chatMessages.channelId} = ${channelId} AND ${chatMessages.pinned} = true AND ${chatMessages.deletedAt} IS NULL`
    )
...
export async function createChatNotification(data: InsertChatNotifications) {
  const db = await getDb();
  if (!db) return null;
...
export async function listUnreadNotifications(userId: number) {
  ...
  return db
    ...
    .where(
      sql`${chatNotifications.userId} = ${userId} AND ${chatNotifications.lida} = false`
    )
...
export async function countUnreadNotifications(userId: number) {
  ...
  if (!db)
    return {
      total: 0,
      byChannel: [] as { channelId: number; count: number }[],
    };
  ...
  const byChannel = ((byChannelRows as any)[0] || []).map((r: any) => ({
...
export async function createColaborador(data: InsertColaboradores) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateColaborador(
  id: number,
  data: Partial<InsertColaboradores>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listFerias(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(ferias)
      .where(eq(ferias.colaboradorId, colaboradorId))
      .orderBy(desc(ferias.createdAt));
  }
...
export async function getFeriasById(id: number) {
  ...
  const result = await db
    .select()
    .from(ferias)
    .where(eq(ferias.id, id))
    .limit(1);
  return result.length > 0 ? result[0] : null;
}

export async function createFerias(data: InsertFeriases) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateFerias(id: number, data: Partial<InsertFeriases>) {
  const db = await getDb();
  if (!db) return;
...
export async function listSolicitacoesFolga(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(solicitacoesFolga)
      .where(eq(solicitacoesFolga.colaboradorId, colaboradorId))
      .orderBy(desc(solicitacoesFolga.createdAt));
  }
  return db
    .select()
    .from(solicitacoesFolga)
...
export async function createSolicitacaoFolga(data: InsertSolicitacoesFolga) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateSolicitacaoFolga(
  id: number,
  data: Partial<InsertSolicitacoesFolga>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(solicitacoesFolga)
    .set(data)
    .where(eq(solicitacoesFolga.id, id));
}
...
export async function listTarefasSetor(setorId?: number) {
  ...
  if (setorId) {
    return db
      .select()
      .from(tarefasSetor)
      .where(eq(tarefasSetor.setorId, setorId))
      .orderBy(desc(tarefasSetor.createdAt));
  }
  return db.select().from(tarefasSetor).orderBy(desc(tarefasSetor.createdAt));
}

export async function createTarefaSetor(data: InsertTarefasSetor) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateTarefaSetor(
  id: number,
  data: Partial<InsertTarefasSetor>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listAcoesBeneficios() {
  ...
  return db
    .select()
    .from(acoesBeneficios)
    .orderBy(desc(acoesBeneficios.createdAt));
}

export async function createAcaoBeneficio(data: InsertAcoesBeneficios) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateAcaoBeneficio(
  id: number,
  data: Partial<InsertAcoesBeneficios>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listAtestadosLicencas(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(atestadosLicencas)
      .where(eq(atestadosLicencas.colaboradorId, colaboradorId))
      .orderBy(desc(atestadosLicencas.createdAt));
  }
  return db
    .select()
    .from(atestadosLicencas)
...
export async function createAtestadoLicenca(data: InsertAtestadosLicencas) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateAtestadoLicenca(
  id: number,
  data: Partial<InsertAtestadosLicencas>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(atestadosLicencas)
    .set(data)
    .where(eq(atestadosLicencas.id, id));
}
...
export async function listPlanosCarreira(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(planosCarreira)
      .where(eq(planosCarreira.colaboradorId, colaboradorId))
      .orderBy(desc(planosCarreira.createdAt));
  }
  return db
    .select()
    .from(planosCarreira)
...
export async function createPlanoCarreira(data: InsertPlanosCarreira) {
  const db = await getDb();
  if (!db) return null;
...
export async function updatePlanoCarreira(
  id: number,
  data: Partial<InsertPlanosCarreira>
) {
  const db = await getDb();
  if (!db) return;
...
import {
  ciclosAvaliacao,
  InsertCicloAvaliacao,
  avaliacoes,
  InsertAvaliacao,
  colaboradorDocumentos,
  InsertColaboradorDocumento,
} from "../drizzle/schema";
...
export async function listCiclosAvaliacao() {
  ...
  return db
    .select()
    .from(ciclosAvaliacao)
    .orderBy(desc(ciclosAvaliacao.createdAt));
}
...
export async function getCicloAvaliacaoById(id: number) {
  ...
  const result = await db
    .select()
    .from(ciclosAvaliacao)
    .where(eq(ciclosAvaliacao.id, id))
    .limit(1);
  return result.length > 0 ? result[0] : null;
}
...
export async function updateCicloAvaliacao(
  id: number,
  data: Partial<InsertCicloAvaliacao>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listAvaliacoes(cicloId?: number, colaboradorId?: number) {
  ...
  if (cicloId && colaboradorId) {
    return db
      ...
      .where(
        and(
          eq(avaliacoes.cicloId, cicloId),
          eq(avaliacoes.colaboradorId, colaboradorId)
        )
      )
  ...
  if (cicloId) {
    return db
      .select()
      .from(avaliacoes)
      .where(eq(avaliacoes.cicloId, cicloId))
      .orderBy(desc(avaliacoes.createdAt));
  }
  if (colaboradorId) {
    return db
      .select()
      .from(avaliacoes)
      .where(eq(avaliacoes.colaboradorId, colaboradorId))
      .orderBy(desc(avaliacoes.createdAt));
  }
...
export async function listColaboradorDocumentos(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(colaboradorDocumentos)
      .where(eq(colaboradorDocumentos.colaboradorId, colaboradorId))
      .orderBy(desc(colaboradorDocumentos.createdAt));
  }
  return db
    .select()
    .from(colaboradorDocumentos)
...
export async function createColaboradorDocumento(
  data: InsertColaboradorDocumento
) {
  const db = await getDb();
  if (!db) return null;
...
export async function deleteColaboradorDocumento(id: number) {
  ...
  if (!db) return;
  await db
    .delete(colaboradorDocumentos)
    .where(eq(colaboradorDocumentos.id, id));
}

import { metasIndividuais, InsertMetaIndividual } from "../drizzle/schema";

// ---- METAS INDIVIDUAIS (KPIs) ----
export async function listMetasIndividuais(
  ...
  if (colaboradorId && cicloId) {
    return db
      .select()
      .from(metasIndividuais)
      .where(
        and(
          eq(metasIndividuais.colaboradorId, colaboradorId),
          eq(metasIndividuais.cicloId, cicloId)
        )
      )
      .orderBy(desc(metasIndividuais.createdAt));
  }
  if (colaboradorId) {
    return db
      .select()
      .from(metasIndividuais)
      .where(eq(metasIndividuais.colaboradorId, colaboradorId))
      .orderBy(desc(metasIndividuais.createdAt));
  }
  if (cicloId) {
    return db
      .select()
      .from(metasIndividuais)
      .where(eq(metasIndividuais.cicloId, cicloId))
      .orderBy(desc(metasIndividuais.createdAt));
  }
  return db
    .select()
...
export async function getMetaIndividualById(id: number) {
  ...
  const result = await db
    .select()
    .from(metasIndividuais)
    .where(eq(metasIndividuais.id, id))
    .limit(1);
  return result.length > 0 ? result[0] : null;
}
...
export async function updateMetaIndividual(
  id: number,
  data: Partial<InsertMetaIndividual>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(metasIndividuais)
    .set(data)
    .where(eq(metasIndividuais.id, id));
}
...
export async function notificarCicloAberto(
  cicloTitulo: string,
  cicloId: number
) {
  const db = await getDb();
...
export async function notificarAvaliacaoPendente(
  colaboradorNome: string,
  avaliadorUserId: number | null,
  cicloTitulo: string
) {
  const db = await getDb();
...
export async function listHistoricoStatus(colaboradorId: number) {
  ...
  return db
    .select()
    .from(historicoStatusColaborador)
    .where(eq(historicoStatusColaborador.colaboradorId, colaboradorId))
    .orderBy(desc(historicoStatusColaborador.createdAt));
}

export async function createHistoricoStatus(
  data: InsertHistoricoStatusColaboradores
) {
  const db = await getDb();
  if (!db) return null;
...
export async function changeColaboradorStatus(
  ...
  const [colab] = await db
    .select({ statusColaborador: colaboradores.statusColaborador })
    .from(colaboradores)
...
export async function getContratosVencendo(diasAntecedencia: number = 30) {
  ...
  for (const c of rows) {
    ...
    const periodos = [
      {
        inicio: c.periodoExperiencia1Inicio,
        fim: c.periodoExperiencia1Fim,
        label: "1º Período",
      },
      {
        inicio: c.periodoExperiencia2Inicio,
        fim: c.periodoExperiencia2Fim,
        label: "2º Período",
      },
    ];
...
export async function registrarEmailAniversarianteEnviado(
  colaboradorId: number,
  ano: number
) {
  const db = await getDb();
...
export async function registrarNotificacaoAniversarioAntecipada(
  colaboradorId: number,
  ano: number,
  diasAntes: number
) {
...
export async function getNotificacoesAniversarioEnviadas(
  ano: number,
  diasAntes: number
) {
  const db = await getDb();
...
export async function updateWorkflowRenovacao(
  id: number,
  data: { status: string; decisao?: string; observacao?: string }
) {
  const db = await getDb();
  if (!db) return;
  if (data.decisao) {
...
export async function createOnboardingTemplate(
  data: InsertOnboardingTemplates
) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateOnboardingTemplate(
  id: number,
  data: Partial<InsertOnboardingTemplates>
) {
  const db = await getDb();
  if (!db) return;
  await db
...
export async function deleteOnboardingTemplate(id: number) {
  ...
  if (!db) return;
  await db
    .delete(onboardingEtapasTemplate)
    .where(eq(onboardingEtapasTemplate.templateId, id));
...
export async function createEtapaTemplate(
  data: InsertOnboardingEtapasTemplate
) {
  const db = await getDb();
  if (!db) return null;
...
export async function deleteEtapaTemplate(id: number) {
  ...
  await db
    .delete(onboardingEtapasTemplate)
    .where(eq(onboardingEtapasTemplate.id, id));
}
...
export async function createOnboardingColaborador(
  data: InsertOnboardingColaboradores
) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateOnboardingColaborador(
  id: number,
  data: Partial<InsertOnboardingColaboradores>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(onboardingColaborador)
    .set(data)
    .where(eq(onboardingColaborador.id, id));
}
...
export async function listOnboardingEtapas(onboardingId: number) {
  ...
  return db
    .select()
    .from(onboardingEtapas)
    .where(eq(onboardingEtapas.onboardingId, onboardingId))
    .orderBy(onboardingEtapas.ordem);
}

export async function createOnboardingEtapa(data: InsertOnboardingEtapas) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateOnboardingEtapa(
  id: number,
  data: Partial<InsertOnboardingEtapas>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(onboardingEtapas)
    .set(data)
    .where(eq(onboardingEtapas.id, id));
}
...
export async function listPesquisasClima() {
  ...
  return db
    .select()
    .from(pesquisasClima)
    .orderBy(desc(pesquisasClima.createdAt));
}

export async function createPesquisaClima(data: InsertPesquisasClima) {
  const db = await getDb();
  if (!db) return null;
...
export async function updatePesquisaClima(
  id: number,
  data: Partial<InsertPesquisasClima>
) {
  const db = await getDb();
  if (!db) return;
...
export async function createPerguntaClima(data: InsertPerguntasClima) {
  const db = await getDb();
  if (!db) return null;
...
export async function submitRespostasClima(
  pesquisaId: number,
  respostas: InsertRespostasClima[]
) {
  const db = await getDb();
  if (!db) return;
...
export async function updateBancoHoras(
  id: number,
  data: Partial<InsertBancoHoras>
) {
  const db = await getDb();
  if (!db) return;
...
export async function getSaldoBancoHoras(colaboradorId: number) {
  ...
  const registros = await db
    ...
    .where(
      and(
        eq(bancoHoras.colaboradorId, colaboradorId),
        eq(bancoHoras.aprovado, true)
      )
    );
  let extras = 0;
  let compensacoes = 0;
...
export async function getSaldosBancoHorasAll() {
  ...
  const registros = await db
    .select()
    .from(bancoHoras)
    .where(eq(bancoHoras.aprovado, true));
  const map: Record<
    number,
    {
      colaboradorId: number;
  ...
  for (const r of registros) {
    ...
    const h = Number(r.horas);
    if (r.tipo === "extra" || r.tipo === "ajuste_positivo")
      map[r.colaboradorId].extras += h;
    else map[r.colaboradorId].compensacoes += h;
  }
  return Object.values(map).map(m => ({
    ...m,
    saldo: m.extras - m.compensacoes,
  }));
}
...
export async function findOrCreateDmChannel(
  ...
  const existing = await db
    ...
    .from(chatChannels)
    .where(
      sql`${chatChannels.tipo} = 'dm' AND ${chatChannels.dmUser1Id} = ${uid1} AND ${chatChannels.dmUser2Id} = ${uid2} AND ${chatChannels.status} != 'deleted'`
    );
  ...
  const rows = await db
    .select()
    .from(chatChannels)
    .where(sql`${chatChannels.id} = ${newId}`);
  return rows[0] || null;
}
...
export async function listDmChannelsForUser(userId: number) {
  ...
  return db
    ...
    .from(chatChannels)
    .where(
      sql`${chatChannels.tipo} = 'dm' AND (${chatChannels.dmUser1Id} = ${userId} OR ${chatChannels.dmUser2Id} = ${userId}) AND ${chatChannels.status} != 'deleted'`
    )
...
export async function getTypingUsers(channelId: number, excludeUserId: number) {
  ...
  return db
    ...
    .where(
      sql`${chatTypingIndicators.channelId} = ${channelId} AND ${chatTypingIndicators.userId} != ${excludeUserId} AND ${chatTypingIndicators.startedAt} > DATE_SUB(NOW(), INTERVAL 5 SECOND)`
    );
}
...
export async function searchChatFiles(
  ...
  if (channelId && fileType) {
    ...
    return db
      ...
      .from(chatMessages)
      .where(
        sql`${chatMessages.channelId} = ${channelId} AND ${chatMessages.fileUrl} IS NOT NULL AND ${chatMessages.fileUrl} != '' AND ${chatMessages.fileType} LIKE ${likeType} AND ${chatMessages.deletedAt} IS NULL`
      )
      .orderBy(desc(chatMessages.id))
      .limit(limit);
  }
  if (channelId) {
    return db
      ...
      .from(chatMessages)
      .where(
        sql`${chatMessages.channelId} = ${channelId} AND ${chatMessages.fileUrl} IS NOT NULL AND ${chatMessages.fileUrl} != '' AND ${chatMessages.deletedAt} IS NULL`
      )
      .orderBy(desc(chatMessages.id))
      .limit(limit);
  }
  if (fileType) {
    ...
    return db
      ...
      .from(chatMessages)
      .where(
        sql`${chatMessages.fileUrl} IS NOT NULL AND ${chatMessages.fileUrl} != '' AND ${chatMessages.fileType} LIKE ${likeType} AND ${chatMessages.deletedAt} IS NULL`
      )
      .orderBy(desc(chatMessages.id))
      .limit(limit);
  }
  return db
    ...
    .from(chatMessages)
    .where(
      sql`${chatMessages.fileUrl} IS NOT NULL AND ${chatMessages.fileUrl} != '' AND ${chatMessages.deletedAt} IS NULL`
    )
    .orderBy(desc(chatMessages.id))
    .limit(limit);
}

export async function searchChatMessagesByUser(
  channelId: number,
  userName: string,
  limit = 50
) {
  ...
  return db
    ...
    .from(chatMessages)
    .where(
      sql`${chatMessages.channelId} = ${channelId} AND ${chatMessages.userName} LIKE ${like} AND ${chatMessages.deletedAt} IS NULL`
    )
    .orderBy(desc(chatMessages.id))
    .limit(limit);
}
...
export async function getThreadCount(messageIds: number[]) {
  ...
  if (!db) return [];
  const rows = await db.execute(
    sql`SELECT replyToId, COUNT(*) as cnt FROM chat_messages WHERE replyToId IN (${sql.join(
      messageIds.map(id => sql`${id}`),
      sql`, `
    )}) AND deletedAt IS NULL GROUP BY replyToId`
  );
  return ((rows as any)[0] || []).map((r: any) => ({
    messageId: Number(r.replyToId),
    count: Number(r.cnt),
  }));
}
...
export async function getPartnerCommissionsDashboard(parceiroId: number) {
  ...
  const clientesVinculados = await db
    ...
    .from(clientes)
    .where(eq(clientes.parceiroId, parceiroId));
  ...
  const aprovacoes = await db
    .select()
    .from(aprovacaoComissao)
    .where(eq(aprovacaoComissao.parceiroId, parceiroId))
    .orderBy(desc(aprovacaoComissao.createdAt))
    .limit(10);

  // Get subpartners if this is a main partner
  const subparceiros = await db
    .select({
      id: parceiros.id,
      nomeCompleto: parceiros.nomeCompleto,
      apelido: parceiros.apelido,
    ...
    .where(
      and(
        eq(parceiros.parceiroPaiId, parceiroId),
        eq(parceiros.ehSubparceiro, 1)
      )
    );

  // Get rateio info if subpartner
  const rateios =
    parceiro.ehSubparceiro && parceiro.parceiroPaiId
      ? await db
          .select()
          .from(rateioComissao)
          .where(eq(rateioComissao.parceiroId, parceiroId))
  ...
  const modelo = parceiro.modeloParceriaId
    ? await db
        .select()
        .from(modelosParceria)
        .where(eq(modelosParceria.id, parceiro.modeloParceriaId))
        .limit(1)
    : [];
...
export async function getConsolidatedCommissionsDashboard(filters?: {
  ...
  if (filters?.tipoParceiro) {
    allParceiros = allParceiros.filter(
      (p: any) => p.tipoPessoa === filters.tipoParceiro
    );
  }
  ...
  if (filters?.modeloParceriaId) {
    allParceiros = allParceiros.filter(
      (p: any) => p.modeloParceriaId === filters.modeloParceriaId
    );
  }
  ...
  // All aprovacoes
  let allAprovacoes = await db
    .select()
    .from(aprovacaoComissao)
    .orderBy(desc(aprovacaoComissao.createdAt));

  // Filter aprovacoes by parceiro ids
  allAprovacoes = allAprovacoes.filter((a: any) =>
    parceiroIds.has(a.parceiroId)
  );

  // Apply date range filter on aprovacoes
  if (filters?.dataInicio) {
    const startDate = new Date(filters.dataInicio);
    allAprovacoes = allAprovacoes.filter(
      (a: any) => new Date(a.createdAt) >= startDate
    );
  }
  if (filters?.dataFim) {
    const endDate = new Date(filters.dataFim + "T23:59:59");
    allAprovacoes = allAprovacoes.filter(
      (a: any) => new Date(a.createdAt) <= endDate
    );
  }
  ...
  const allClientes = await db
    .select()
    .from(clientes)
    .where(isNotNull(clientes.parceiroId));
  ...
  const parceiroStats = allParceiros.map(p => {
    ...
    const pServicos = allParceiroServicos.filter(
      (s: any) => s.parceiroId === p.id
    );
    ...
    const totalValor = aprovadas.reduce(
      (sum: number, a: any) => sum + Number(a.valorComissao || 0),
      0
    );
  ...
  const totalAprovadas = allAprovacoes.filter(
    (a: any) => a.status === "aprovada"
  ).length;
  const totalPendentes = allAprovacoes.filter(
    (a: any) => a.status === "pendente"
  ).length;
  const totalRejeitadas = allAprovacoes.filter(
    (a: any) => a.status === "rejeitada"
  ).length;
...
export async function updateMetaComissao(
  id: number,
  data: Partial<InsertMetaComissao>
) {
  const db = await getDb();
  if (!db) return;
...
export async function getMetasComissoesByPeriod(ano: number, mes?: number) {
  ...
  return db.select().from(metasComissoes).where(eq(metasComissoes.ano, ano));
}

// Get partner goal progress: meta vs realizado
export async function getMetaProgressoParceiro(
  parceiroId: number,
  ano: number,
  mes?: number
) {
  ...
  // Get the meta
  const conditions = [
    eq(metasComissoes.parceiroId, parceiroId),
    eq(metasComissoes.ano, ano),
  ];
  ...
  const realizado = filtered.reduce(
    (sum: number, a: any) => sum + Number(a.percentualSolicitado || 0),
    0
  );
...
export async function getAllMetasProgresso(ano: number, mes?: number) {
  ...
  // Get parceiro names
  const allParceiros = await db
    .select({
      id: parceiros.id,
      nome: parceiros.nomeCompleto,
      apelido: parceiros.apelido,
    })
    .from(parceiros);
...
export async function getComparativoPeriodos(
  ...
  if (tipoParceiro)
    allParceiros = allParceiros.filter(
      (p: any) => p.tipoPessoa === tipoParceiro
    );
  if (modeloParceriaId)
    allParceiros = allParceiros.filter(
      (p: any) => p.modeloParceriaId === modeloParceriaId
    );
  const parceiroIds = new Set(allParceiros.map(p => p.id));
  ...
  const calcKpis = (aprovs: any[]) => {
    ...
    return {
      ...
      valorTotalAprovado: aprovadas.reduce(
        (sum: number, a: any) => sum + Number(a.valorComissao || 0),
        0
      ),
      valorTotalPendente: pendentes.reduce(
        (sum: number, a: any) => sum + Number(a.valorComissao || 0),
        0
      ),
    };
  ...
  const deltas = {
    ...
    valorTotalAprovado: calcDelta(
      kpisA.valorTotalAprovado,
      kpisB.valorTotalAprovado
    ),
    valorTotalPendente: calcDelta(
      kpisA.valorTotalPendente,
      kpisB.valorTotalPendente
    ),
  };
  ...
  const calcRankingPeriod = (aprovs: any[]) => {
    const map: Record<
      number,
      { total: number; aprovadas: number; valor: number }
    > = {};
    for (const a of aprovs) {
      if (!map[a.parceiroId])
        map[a.parceiroId] = { total: 0, aprovadas: 0, valor: 0 };
  ...
  // Merge rankings
  const allParceiroIds = new Set(
    [...Object.keys(rankA), ...Object.keys(rankB)].map(Number)
  );
  const rankingComparativo = Array.from(allParceiroIds)
    .map(pid => {
...
export async function listValeTransporte(mesRef?: number, anoRef?: number) {
  ...
  if (mesRef && anoRef) {
    return db
      ...
      .where(
        and(
          eq(valeTransporte.mesReferencia, mesRef),
          eq(valeTransporte.anoReferencia, anoRef)
        )
      )
...
export async function createValeTransporte(data: InsertValeTransportes) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateValeTransporte(
  id: number,
  data: Partial<InsertValeTransportes>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listAcademiaBeneficio(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(academiaBeneficio)
      .where(eq(academiaBeneficio.colaboradorId, colaboradorId))
      .orderBy(desc(academiaBeneficio.createdAt));
  }
  return db
    .select()
    .from(academiaBeneficio)
...
export async function updateAcademiaBeneficio(
  id: number,
  data: Partial<InsertAcademiaBeneficio>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(academiaBeneficio)
    .set(data)
    .where(eq(academiaBeneficio.id, id));
}
...
export async function listComissaoRh(mesRef?: number, anoRef?: number) {
  ...
  if (mesRef && anoRef) {
    return db
      ...
      .where(
        and(
          eq(comissaoRh.mesReferencia, mesRef),
          eq(comissaoRh.anoReferencia, anoRef)
        )
      )
...
export async function createComissaoRh(data: InsertComissoesRh) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateComissaoRh(
  id: number,
  data: Partial<InsertComissoesRh>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listDayOff(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(dayOff)
      .where(eq(dayOff.colaboradorId, colaboradorId))
      .orderBy(desc(dayOff.createdAt));
  }
  return db.select().from(dayOff).orderBy(desc(dayOff.createdAt));
}

export async function createDayOff(data: InsertDaysOff) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateDayOff(id: number, data: Partial<InsertDaysOff>) {
  const db = await getDb();
  if (!db) return;
...
export async function listDoacaoSangue(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(doacaoSangue)
      .where(eq(doacaoSangue.colaboradorId, colaboradorId))
      .orderBy(desc(doacaoSangue.createdAt));
  }
  return db.select().from(doacaoSangue).orderBy(desc(doacaoSangue.createdAt));
}

export async function createDoacaoSangue(data: InsertDoacoesSangue) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateDoacaoSangue(
  id: number,
  data: Partial<InsertDoacoesSangue>
) {
  const db = await getDb();
  if (!db) return;
...
export async function listReajustesSalariais(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(reajustesSalariais)
      .where(eq(reajustesSalariais.colaboradorId, colaboradorId))
      .orderBy(desc(reajustesSalariais.createdAt));
  }
  return db
    .select()
    .from(reajustesSalariais)
...
export async function createReajusteSalarial(data: InsertReajustesSalariais) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateReajusteSalarial(
  id: number,
  data: Partial<InsertReajustesSalariais>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(reajustesSalariais)
    .set(data)
    .where(eq(reajustesSalariais.id, id));
}
...
export async function createApontamentoFolha(data: InsertApontamentosFolha) {
  const db = await getDb();
  if (!db) return null;
...
export async function deleteApontamentosFolhaPorReferencia(
  mesRef: number,
  anoRef: number
) {
  const db = await getDb();
...
export async function listNiveisCargo(setorId?: number) {
  ...
  if (setorId) {
    return db
      .select()
      .from(niveisCargo)
      .where(eq(niveisCargo.setorId, setorId))
      .orderBy(niveisCargo.nivel);
  }
  return db
    .select()
    .from(niveisCargo)
    .orderBy(niveisCargo.setorId, niveisCargo.nivel);
}

export async function createNivelCargo(data: InsertNiveisCargo) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateNivelCargo(
  id: number,
  data: Partial<InsertNiveisCargo>
) {
  const db = await getDb();
  if (!db) return;
...
export async function gerarApontamentosFolha(
  mesRef: number,
  anoRef: number,
  ...
  const apontamentos: InsertApontamentosFolha[] = [];
  ...
  for (const r of reajustes) {
    ...
    const dataEfetivacao = r.dataEfetivacao
      ? new Date(r.dataEfetivacao + "T12:00:00")
      : null;
    if (
      dataEfetivacao &&
      dataEfetivacao.getMonth() + 1 === mesRef &&
      dataEfetivacao.getFullYear() === anoRef
    ) {
      apontamentos.push({
        colaboradorId: r.colaboradorId,
        colaboradorNome: r.colaboradorNome,
        ...
        anoReferencia: anoRef,
        tipo:
          r.tipo === "sindical" ? "reajuste_sindical" : "reajuste_dois_anos",
        descricao: `Reajuste ${r.tipo === "sindical" ? "Sindical" : "2 anos"} - ${r.percentual}% sobre R$${r.salarioAnterior}`,
        valor: String(
          Number(r.salarioNovo || 0) - Number(r.salarioAnterior || 0)
        ),
        registradoPorId,
        registradoPorNome,
      });
  ...
  for (const c of colabs) {
    ...
    if (
      c.temContribuicaoAssistencial &&
      Number(c.valorContribuicaoAssistencial || 0) > 0
    ) {
      apontamentos.push({
        colaboradorId: c.id,
        colaboradorNome: c.nomeCompleto,
...
export async function checkReajusteDoisAnos() {
  ...
  for (const c of colabs) {
    ...
    if (anosCompletos >= 2 && anosCompletos % 2 === 0) {
      ...
      const jaTemReajuste = existingReajustes.some(
        r =>
          r.tipo === "dois_anos" &&
          r.dataEfetivacao &&
          new Date(r.dataEfetivacao + "T12:00:00").getFullYear() ===
            hoje.getFullYear()
      );
      if (!jaTemReajuste) {
        elegíveis.push({
...
export async function getDashboardGEG(mes?: number, ano?: number) {
  ...
  const totalAcademia = academias.reduce(
    (sum, a) => sum + (a.descontoFolha ? Number(a.valorPlano || 0) : 0),
    0
  );
  ...
  const totalComissoes = comissoes.reduce(
    (sum, c) => sum + Number(c.valorComissao || 0),
    0
  );
  ...
  const dayOffsPendentes = dayOffsDoMes.filter(
    d => d.status === "pendente"
  ).length;
  const dayOffsAprovados = dayOffsDoMes.filter(
    d => d.status === "aprovado"
  ).length;
  ...
  for (const c of ativos) {
    ...
    const diffDias = Math.ceil(
      (anivEsteAno.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const mLabel = d.toLocaleDateString("pt-BR", {
      month: "short",
      year: "2-digit",
    });

    const vtMes = await db
      .select()
      .from(valeTransporte)
      .where(
        and(
          eq(valeTransporte.mesReferencia, m),
          eq(valeTransporte.anoReferencia, a)
        )
      );
    const vtTotal = vtMes.reduce((s, v) => s + Number(v.valorTotal || 0), 0);

    const comMes = await db
      .select()
      .from(comissaoRh)
      .where(
        and(eq(comissaoRh.mesReferencia, m), eq(comissaoRh.anoReferencia, a))
      );
    const comTotal = comMes.reduce(
      (s, c) => s + Number(c.valorComissao || 0),
      0
    );
  ...
  const reajustesPendentesLista = elegiveis.map(e => ({
    ...
    salarioEstimado: Number((e.salarioAtual * 1.1).toFixed(2)),
  }));
  ...
  return {
    ...
    reajustesPendentes,
    dayOffs: {
      pendentes: dayOffsPendentes,
      aprovados: dayOffsAprovados,
      total: dayOffsDoMes.length,
    },
...
export async function listBeneficiosCustom() {
  ...
  return db
    .select()
    .from(beneficiosCustom)
    .orderBy(desc(beneficiosCustom.createdAt));
}

export async function createBeneficioCustom(data: InsertBeneficiosCustom) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateBeneficioCustom(
  id: number,
  data: Partial<InsertBeneficiosCustom>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(beneficiosCustom)
    .set(data)
    .where(eq(beneficiosCustom.id, id));
}
...
export async function listProgramasCarreira() {
  ...
  return db
    .select()
    .from(programasCarreira)
    .orderBy(desc(programasCarreira.createdAt));
}

export async function createProgramaCarreira(data: InsertProgramasCarreira) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateProgramaCarreira(
  id: number,
  data: Partial<InsertProgramasCarreira>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(programasCarreira)
    .set(data)
    .where(eq(programasCarreira.id, id));
}
...
export async function createRescisao(data: InsertRescisoes) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateRescisao(
  id: number,
  data: Partial<InsertRescisoes>
) {
  const db = await getDb();
  if (!db) return;
...
export function calcularRescisao(params: {
  ...
  const { salarioBase, dataAdmissao, dataDesligamento, tipoDesligamento } =
    params;
  ...
  const diasNoMes = new Date(
    ...
    desligamento.getMonth() + 1,
    0
  ).getDate();
  const saldoSalario = Number(
    ((salarioBase / 30) * diaDesligamento).toFixed(2)
  );
  ...
  // FGTS
  const fgtsDepositado = Number(
    (salarioBase * 0.08 * mesesTrabalhados).toFixed(2)
  );
  let multaFgts = 0;
  ...
  const isExperiencia =
    ...
    tipoDesligamento === "termino_experiencia_2";
  ...
  switch (tipoDesligamento) {
    case "sem_justa_causa":
      ...
      decimoTerceiroProporcional = Number(
        ((salarioBase / 12) * decimoTerceiroMeses).toFixed(2)
      );
      // Férias proporcionais + 1/3
      feriasMeses = mesesDesdeUltimoAniversario;
      feriasProporcionais = Number(
        ((salarioBase / 12) * feriasMeses).toFixed(2)
      );
      ...
      multaFgts = Number((fgtsDepositado * 0.4).toFixed(2));
      break;
    ...
    case "acordo_mutuo":
      ...
      avisoPrevioDias = Math.min(90, 30 + anosTrabalhados * 3);
      avisoPrevio = Number(
        ((salarioBase / 30) * avisoPrevioDias * 0.5).toFixed(2)
      );
      ...
      multaFgts = Number((fgtsDepositado * 0.2).toFixed(2));
      break;
  }
...
export async function listEquipamentos(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(equipamentosColaborador)
      .where(eq(equipamentosColaborador.colaboradorId, colaboradorId))
      .orderBy(desc(equipamentosColaborador.createdAt));
  }
  return db
    .select()
    .from(equipamentosColaborador)
...
export async function createEquipamento(data: InsertEquipamentosColaborador) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateEquipamento(
  id: number,
  data: Partial<InsertEquipamentosColaborador>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(equipamentosColaborador)
    .set(data)
    .where(eq(equipamentosColaborador.id, id));
}
...
export async function deleteEquipamento(id: number) {
  ...
  if (!db) return;
  await db
    .delete(equipamentosColaborador)
    .where(eq(equipamentosColaborador.id, id));
}
...
export async function listSenhasAutorizacoes(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(senhasAutorizacoes)
      .where(eq(senhasAutorizacoes.colaboradorId, colaboradorId))
      .orderBy(desc(senhasAutorizacoes.createdAt));
  }
  return db
    .select()
    .from(senhasAutorizacoes)
...
export async function createSenhaAutorizacao(data: InsertSenhasAutorizacoes) {
  const db = await getDb();
  if (!db) return null;
...
export async function updateSenhaAutorizacao(
  id: number,
  data: Partial<InsertSenhasAutorizacoes>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(senhasAutorizacoes)
    .set(data)
    .where(eq(senhasAutorizacoes.id, id));
}
...
export async function listSenhaHistorico(senhaAutorizacaoId?: number) {
  ...
  if (senhaAutorizacaoId) {
    return db
      .select()
      .from(senhaHistorico)
      .where(eq(senhaHistorico.senhaAutorizacaoId, senhaAutorizacaoId))
      .orderBy(desc(senhaHistorico.createdAt));
  }
  return db
    .select()
    .from(senhaHistorico)
...
export async function createSenhaHistorico(data: InsertSenhasHistorico) {
  const db = await getDb();
  if (!db) return null;
...
export async function listEmailsCorporativos(colaboradorId?: number) {
  ...
  if (colaboradorId) {
    return db
      .select()
      .from(emailsCorporativos)
      .where(eq(emailsCorporativos.colaboradorId, colaboradorId))
      .orderBy(desc(emailsCorporativos.createdAt));
  }
  return db
    .select()
    .from(emailsCorporativos)
...
export async function createEmailCorporativo(data: InsertEmailsCorporativos) {
  const db = await getDb();
  if (!db) return 0;
...
export async function updateEmailCorporativo(
  id: number,
  data: Partial<InsertEmailsCorporativos>
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(emailsCorporativos)
    .set(data)
    .where(eq(emailsCorporativos.id, id));
}
...
export async function checkEmailExists(email: string): Promise<boolean> {
  ...
  const results = await db
    .select()
    .from(emailsCorporativos)
    .where(eq(emailsCorporativos.email, email));
...
export async function listTermosResponsabilidade(filters?: {
  colaboradorId?: number;
  ...
  if (filters?.colaboradorId)
    conditions.push(
      eq(termosResponsabilidade.colaboradorId, filters.colaboradorId)
    );
  if (filters?.equipamentoId)
    conditions.push(
      eq(termosResponsabilidade.equipamentoId, filters.equipamentoId)
    );
  if (conditions.length > 0) {
    return db
      .select()
...
export async function updateTermoResponsabilidade(id: number, data: any) {
  ...
  if (!db) return;
  await db
    .update(termosResponsabilidade)
    .set(data)
    .where(eq(termosResponsabilidade.id, id));
}
...
export async function getRelatorioAtivosColaborador(colaboradorId?: number) {
  ...
  const conditions = colaboradorId
    ? [eq(equipamentosColaborador.colaboradorId, colaboradorId)]
    : [];
  const conditionsEmail = colaboradorId
    ? [eq(emailsCorporativos.colaboradorId, colaboradorId)]
    : [];
  const conditionsSenha = colaboradorId
    ? [eq(senhasAutorizacoes.colaboradorId, colaboradorId)]
    : [];
  const conditionsTermos = colaboradorId
    ? [eq(termosResponsabilidade.colaboradorId, colaboradorId)]
    : [];
...
export async function getCCTVigente() {
  ...
  const rows = await db
    .select()
    .from(convencaoColetiva)
    .where(eq(convencaoColetiva.status, "vigente"))
    .orderBy(desc(convencaoColetiva.createdAt))
    .limit(1);
  return rows[0] || null;
}
...
export async function listRescisaoAuditoria(limit = 50) {
  ...
  return db
    .select()
    .from(rescisaoAuditoria)
    .orderBy(desc(rescisaoAuditoria.createdAt))
    .limit(limit);
}
...
export function classificarOcorrencia(
  ...
  const isIrreversivel =
    irreversiveis.has(key) ||
    gravidade === "gravissima" ||
    (reincidencias >= 3 && gravidade !== "leve");
...
export async function updateEtapaPlano(id: number, data: any) {
  ...
  if (!db) return;
  await db
    .update(planoReversaoEtapas)
    .set({ ...data, updatedAt: Date.now() })
    .where(eq(planoReversaoEtapas.id, id));
}
...
export async function deleteFeedbackPlano(id: number) {
  ...
  if (!db) return;
  await db
    .delete(planoReversaoFeedbacks)
    .where(eq(planoReversaoFeedbacks.id, id));
}
...
export async function getOcorrenciasStats() {
  ...
  if (!db)
    return {
      ...
      emAnalise: 0,
      resolvidas: 0,
      encaminhadasReversao: 0,
      encaminhadasDesligamento: 0,
      porTipo: {},
      porGravidade: {},
    };
  ...
  for (const o of all) {
    ...
    else if (o.status === "encaminhada_desligamento")
      encaminhadasDesligamento++;
  }
...
export async function getOcorrenciasDashboard() {
  ...
  const sucesso = allPlanos.filter(
    p => p.status === "concluido_sucesso"
  ).length;
  const fracasso = allPlanos.filter(
    p => p.status === "concluido_fracasso"
  ).length;
  const cancelados = allPlanos.filter(p => p.status === "cancelado").length;
  const concluidos = sucesso + fracasso;
  const taxaSucesso =
    concluidos > 0 ? Math.round((sucesso / concluidos) * 100) : 0;

  // Top reincidentes
  const colabMap: Record<
    number,
    { nome: string; setor: string; cargo: string; count: number }
  > = {};
  for (const o of allOcorrencias) {
    if (!colabMap[o.colaboradorId]) {
      colabMap[o.colaboradorId] = {
        nome: o.colaboradorNome,
        setor: o.setor || "",
        cargo: o.cargo || "",
        count: 0,
      };
...
export async function getHistoricoDisciplinar(colaboradorId: number) {
  ...
  const ocorrenciasList = await db
    .select()
    .from(ocorrencias)
    .where(eq(ocorrencias.colaboradorId, colaboradorId))
    .orderBy(desc(ocorrencias.createdAt));

  const planosList = await db
    .select()
    .from(planoReversao)
    .where(eq(planoReversao.colaboradorId, colaboradorId))
    .orderBy(desc(planoReversao.createdAt));

  const reversiveis = ocorrenciasList.filter(
    o => o.classificacao === "reversivel"
  ).length;
  const irreversiveis = ocorrenciasList.filter(
    o => o.classificacao === "irreversivel"
  ).length;
  const planosAtivos = planosList.filter(p => p.status === "ativo").length;
  const planosConcluidos = planosList.filter(
    p => p.status === "concluido_sucesso" || p.status === "concluido_fracasso"
  ).length;
...
export async function checkReincidenciasAlerta(
  limiteReincidencias: number = 3
) {
  const db = await getDb();
  if (!db) return [];
...
export async function gerarNotificacoesOcorrencias() {
  ...
  return {
    planosVencendo: planosVencendoCount,
...
export async function getGestorDoSetor(setorNome: string) {
  ...
  const setor = setoresRows.find(
    (s: any) => s.nome?.toLowerCase() === setorNome.toLowerCase()
  );
...
export async function getGestorRH() {
  ...
  if (!db) return null;
  const setoresRows = await db.select().from(setores);
  const setorRH = setoresRows.find(
    (s: any) =>
...
export async function estimarCustoRescisao(colaboradorId: number) {
  ...
  const mesesTrabalhados = Math.max(
    1,
    Math.round((now.getTime() - admDate.getTime()) / (1000 * 60 * 60 * 24 * 30))
  );

  const saldoSalario = salarioBase;
  const avisoPrevio =
    salarioBase +
    Math.min(Math.floor(mesesTrabalhados / 12), 20) * (salarioBase / 30) * 3;
  ...
  return {
    ...
    decimoTerceiroProporcional:
      Math.round(decimoTerceiroProporcional * 100) / 100,
...
export async function getRelatorioMensalConsolidado(mes: number, ano: number) {
  ...
  for (const o of ocorrenciasMes) {
    porTipo[o.tipo] = (porTipo[o.tipo] || 0) + 1;
    porGravidade[o.gravidade] = (porGravidade[o.gravidade] || 0) + 1;
    porSetor[o.setor || "Não informado"] =
      (porSetor[o.setor || "Não informado"] || 0) + 1;
    porClassificacao[o.classificacao] =
      (porClassificacao[o.classificacao] || 0) + 1;
    porRecomendacao[o.recomendacao] =
      (porRecomendacao[o.recomendacao] || 0) + 1;
  }
  ...
  const contagemPorColab: Record<
    number,
    { nome: string; count: number; setor: string }
  > = {};
  for (const o of todasOcorrencias) {
    if (!contagemPorColab[o.colaboradorId]) {
      contagemPorColab[o.colaboradorId] = {
        nome: o.colaboradorNome,
        count: 0,
        setor: o.setor || "N/A",
      };
...
export async function getSetorTaskStats(
  setorId: number,
  filters?: {
    responsavelId?: number;
  ...
  if (filters?.periodoInicio)
    conditions.push(sql`${tarefas.createdAt} >= ${filters.periodoInicio}`);
  if (filters?.periodoFim)
    conditions.push(sql`${tarefas.createdAt} <= ${filters.periodoFim}`);
  ...
  const responsavelStats = await db
    .select({
  ...
  const prioridadeBreakdown = await db
    .select({
  ...
  return {
    ...
    pendentes:
      Number(stats?.a_fazer || 0) +
      Number(stats?.em_andamento || 0) +
      Number(stats?.revisao || 0),
